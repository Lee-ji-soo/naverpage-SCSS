...
for (i = 0; i < subArticleList.length; i++) {
    ...
    let subby = subtxt.appendChild(pby);
    let subintro = subtxt.insertBefore(pintro, subby);
    let subtitle = subtxt.insertBefore(ptitle, subintro);
    let subtype = subtxt.insertBefore(ptype, subintro);

    let subdate = subby.appendChild(spandate);
    let subwriter = subby.insertBefore(spanwriter, subdate);

    sublist.classList.add('img', 'subimg');
    subimg.classList.add('txt,subtxt');
    subtype.classList.add('type,subtype');
    subtitle.classList.add('title,subtitle');
    subintro.classList.add('title,subintro');
    subby.classList.add('by,subby');
    subwriter.classList.add('writer,suwriter');
    subdate.classList.add('date,subdate');

    subimg.src = subArticleList[i].img;
    ...
    subby.innerHTML = subArticleList[i].by;
